...
for ix=1:Navg
    ...
    P_vec(ix,:)=V_vec;
    E_vec(ix,:)=WW_vec(NI+1:NN,1)'*INP(NI+1:NN,:);
    I_vec(ix,:)=WW_vec(1:NI,1)'*INP(1:NI,:);
end
...
PE=mean(E_vec,1);
PI=mean(I_vec,1);
vv=plot(xt,PSP,'g','DisplayName','V(t) ','Linewidth',2);hold on;
ii=plot(xt,PE,'r','DisplayName','excitatory input','Linewidth',1.9);hold on;
ee=plot(xt,PI,'b','DisplayName','inhibitory input','Linewidth',1.9);
hold on
...
legend([vv ee ii ],'Location','northwest' )
title('(c)')
